...
#This GUI is used to launch, quit, and display the status of the
#Rclone web GUI command.
...
class NemoRcloneSyncProvider(GObject.GObject, Nemo.MenuProvider, Nemo.NameAndDescProvider):
    def __init__(self):
        ...
        self.currentSync = [None, None, None]

        #Setup the Async command executer
        self.spawn = GAsyncSpawn()
        self.spawn.connect("process-done", self._on_process_done)
        self.spawn.connect("stdout-data", self._on_stdout_data)
        self.spawn.connect("stderr-data", self._on_stdout_data)
    ...
    def on_menu_other_activated(self, menu, parent, folder):
        ...
        if resp == Gtk.ResponseType.OK:
            ...
            #"places" contains a dictionary indexed by place name (in uppercase)
            self.meta_object_cache["places"][label.upper()] = {"label":label, "path":str(path), "first_sync":True}
            self.write_meta_file(folder, self.meta_object_cache)

            #Start a sync using the selected remote
            self.on_sync_requested(None, parent, str(folder), str(path), label.upper(), True)
    ...
    def on_sync_requested(self, menu, parent, folder1, folder2, label, first_sync=False):
        ...
        #These "flags" keep track of the sync currently in progress
        self.currentSync[0] = folder1
        self.currentSync[1] = folder2
        self.currentSync[2] = label
        ...
        try:
            ...
            self.syncDialog.print_line("ERROR: " + str(e) + "\n")
    ...
    def _on_process_done(self, sender, retval):
        ...
        if retval == 0:
            ...
            if self.meta_object_cache["places"][self.currentSync[2]]["first_sync"]:
                self.meta_object_cache["places"][self.currentSync[2]]["first_sync"] = False
    ...
    def _on_stderr_data(self, sender, line):
        pass
        ...
        if "places" in self.meta_object_cache:
            ...
            for key, p in places.items():
                ...
                if ("label" in p) and ("path" in p):
                    ...
                    #Retrieve the first_sync flag from metadata
                    first_sync = p["first_sync"] if ("first_sync" in p) else True

                    sub_menuitem.connect('activate', self.on_sync_requested, window, str(folder_uri), str(p["path"]), key, first_sync)
